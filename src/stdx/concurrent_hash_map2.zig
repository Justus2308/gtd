...
pub fn ConcurrentAutoArrayHashMapUnmanaged(comptime K: type, comptime V: type) type {
    return ConcurrentArrayHashMapUnmanaged(K, V, AutoContext(K), !autoEqlIsCheap(K));
}

/// An `ArrayHashMapUnmanaged` with strings as keys.
pub fn ConcurrentStringArrayHashMapUnmanaged(comptime V: type) type {
    return ConcurrentArrayHashMapUnmanaged([]const u8, V, StringContext, true);
}

pub const StringContext = std.array_hash_map.StringContext;
pub const eqlString = std.array_hash_map.eqlString;
...
pub fn ConcurrentArrayHashMapUnmanaged(
    ...
    comptime prealloc_item_count: usize,
) type {
    return struct {
        ...
        const real_prealloc_item_count =
            if (prealloc_item_count > 0) math.ceilPowerOfTwoAssert(usize, prealloc_item_count) else 0;

        pub const HashList = std.ArrayListUnmanaged(Hash);
        /// The SegmentList type backing this map.
        /// To keep things threadsafe, this list can never shrink.
        pub const KvList = std.SegmentedList(KV, real_prealloc_item_count);
...
pub const getHashPtrAddrFn = std.array_hash_map.getHashPtrAddrFn;
pub const getTrivialEqlFn = std.array_hash_map.getTrivialEqlFn;
pub const AutoContext = std.array_hash_map.AutoContext;
pub const getAutoHashFn = std.array_hash_map.getAutoHashFn;
pub const getAutoEqlFn = std.array_hash_map.getAutoEqlFn;
pub const autoEqlIsCheap = std.array_hash_map.autoEqlIsCheap;
pub const getAutoHashStratFn = std.array_hash_map.getAutoHashStratFn;
