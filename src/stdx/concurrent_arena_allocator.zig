...
/// This allocator takes an existing thread-safe allocator, wraps it, and provides an
/// interface where you can allocate and then free it all together. Calls to free an
/// individual item only free the item if it was the most recent allocation, otherwise
/// calls to free do nothing.
pub const ArenaAllocator = struct {
    ...
    pub const State = struct {
        cur_buf: std.atomic.Value(?[*]u8) = .init(null),
        cur_cap: std.atomic.Value(usize) = .init(0),
        end_index: std.atomic.Value(usize) = .init(0),

        cur_allocating: std.atomic.Value(?[*]u8) = .init(null),
    ...
    const BufNode = List.Node;
    const buf_node_alignment: mem.Alignment = .fromByteUnits(@alignOf(BufNode));
    ...
    pub fn deinit(self: ArenaAllocator) void {
        ...
        while (it) |node| {
            ...
            self.child_allocator.rawFree(alloc_buf, buf_node_alignment, @returnAddress());
            it = next_it;
        }
    }
    ...
    pub fn reset(self: *ArenaAllocator, mode: ResetMode) bool {
        ...
        if (maybe_first_node) |first_node| {
            ...
            if (self.child_allocator.rawResize(first_alloc_buf, buf_node_alignment, total_size, @returnAddress())) {
                // successful resize
                first_node.data = total_size;
            } else {
                // manual realloc
                const new_ptr = self.child_allocator.rawAlloc(total_size, buf_node_alignment, @returnAddress()) orelse {
                    // we failed to preheat the arena properly, signal this to the user.
                    return false;
                };
                self.child_allocator.rawFree(first_alloc_buf, buf_node_alignment, @returnAddress());
    ...
    fn prependToBufList(self: *ArenaAllocator, buf_node: *BufNode) void {
        var next_node = @atomicLoad(?*BufNode, &self.state.buffer_list.first, .monotonic);
        buf_node.next = next_node;
        while (@cmpxchgWeak(?*BufNode, &self.state.buffer_list.first, next_node, buf_node, .acq_rel, .monotonic)) |old_node| {
            next_node = old_node;
            buf_node.next = next_node;
        }
    }
    ...
    fn createNode(self: *ArenaAllocator, prev_len: usize, minimum_size: usize) ?*BufNode {
        ...
        const ptr = self.child_allocator.rawAlloc(len, buf_node_alignment, @returnAddress()) orelse
            return null;

        // busy-wait, should't take too long
        while (self.state.cur_allocating.cmpxchgWeak(null, ptr, .release, .monotonic)) {}
        // now we are the only thread currently allocating

        self.prependToBufList(buf_node);
        self.state.end_index = 0;
        return buf_node;
    }
    ...
    fn alloc(ctx: *anyopaque, n: usize, alignment: mem.Alignment, ra: usize) ?[*]u8 {
        ...
        var cur_node = if (@atomicLoad(?*BufNode, &self.state.buffer_list.first, .acquire)) |first_node|
            first_node
        else
        ...
        while (true) {
            ...
            const cur_end_index = self.state.end_index.load(.acquire);
            const addr = @intFromPtr(cur_buf.ptr) + cur_end_index;
            const adjusted_addr = mem.alignForward(usize, addr, ptr_align);
            const adjusted_index = cur_end_index + (adjusted_addr - addr);
            ...
            if (self.child_allocator.rawResize(cur_alloc_buf, buf_node_alignment, bigger_buf_size, @returnAddress())) {
