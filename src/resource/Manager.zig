...
const Cell = struct {
    ...
    // Credit to https://ziggit.dev/t/padding-struct-to-fixed-size-with-usable-bytes/10144/4
    const last_used_byte_offset = offset: {
        var max: usize = 0;
        for (std.meta.fieldNames(Cell)) |field_name| {
            const end = (@offsetOf(Cell, field_name) + @sizeOf(@FieldType(Cell, field_name)));
            max = @max(max, end);
        }
        break :offset max;
    };
    const padding_size = (@sizeOf(Cell) - last_used_byte_offset);

    fn paddingBytes(cell: *Cell) *[Cell.padding_size]u8 {
        const bytes = mem.asBytes(cell);
        return bytes[Cell.last_used_byte..@sizeOf(Cell)];
    }

    fn bufferFallback(cell: *Cell, allocator: Allocator) stdx.BufferFallbackAllocator {
        return .init(cell.paddingBytes(), allocator);
    }

    pub fn load(cell: *Cell, allocator: Allocator) Loader.Error!void {
        ...
        loop: switch (state) {
            ...
            .unloading => {
                @branchHint(.unlikely);
                std.Thread.Futex.wait(&cell.state, .unloading);
    ...
    pub fn unload(cell: *Cell, allocator: Allocator) bool {
        ...
        loop: switch (state) {
            ...
            .loading => {
                @branchHint(.unlikely);
                std.Thread.Futex.wait(&cell.state, .loading);
                state = cell.state.load(.monotonic);
                continue :loop state;
            },
    ...
    pub fn addReference(cell: *Cell) bool {
        ...
        loop: switch (state) {
            ...
            .max_ref_count => {
                @branchHint(.cold);
                log.warn("reached max reference count for cell ({s})", .{
                    cell.getBytes(),
                });
                return false;
            },
    ...
    comptime {
        assert(@alignOf(Cell) == atomic.cache_line);
        assert(@mod(@sizeOf(Cell), atomic.cache_line) == 0);
        assert(mem.alignForward(usize, Cell.last_used_byte_offset, atomic.cache_line) == @sizeOf(Cell));
    }
};
...
pub fn initInstance(
    ...
    m.arena_mt = std.heap.ThreadSafeAllocator(m.arena.allocator());
...
pub fn unload(m: *Manager, gpa: Allocator, handle: Loader.Handle) bool {
    ...
    return allocatable.cell.unload(gpa);
}
